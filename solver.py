...
class Solver:
    ...
    def solve(parsed):
            ...
            for flaskNum in range(len(flasks)):
                ...
                for otherFlaskNum in range(len(flasks)):
                    if otherFlaskNum == flaskNum or flasks[otherFlaskNum].isFull:
                        continue
                    if flasks[otherFlaskNum].isEmpty or (
                            not flasks[otherFlaskNum].isFull and flasks[otherFlaskNum].isLastValueEqual(lastItem)):
                        # print('Empty flask')
        ...
        while solvedState != None:
            ...
            #     continue
            # print([",".join([y for y in name if name[y] == x]) + "Колба:" + str(comPared + 1) for x in
            #        solvedState.getFlasks()[comPared].getStack()])
            res.append(solvedState.toDict())
            solvedState = solvedState.parentState
